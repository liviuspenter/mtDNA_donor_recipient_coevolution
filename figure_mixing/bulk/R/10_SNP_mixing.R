...
SNP.count = data.table::fread('./data/bulk/PRJNA741686/SNP_count.csv')
...
ggsave('./figures/bulk/20230330_expressed_SNPs_per_individual.svg', width = 2, height = 2)

SNP.data = data.table::fread('./data/bulk/PRJNA741686/simulated_snps.csv.gz')
...
ggplot(SNP.data, aes(x=sum)) + geom_histogram(binwidth = 500, fill='lightblue') + 
  ...
  theme(legend.position = 'none',
        axis.title = element_text('Arial', size=10, color='black'),
        axis.text = element_text('Arial', size=10, color='black'))
ggsave('./figures/bulk/20230330_informative_SNPs_per_pair.svg', width = 2, height = 2)