...
ggplot(coverage, aes(x=pos, y=coverage.mean/1000)) + 
  ...
  scale_fill_manual(values = BuenColors::jdb_palette(name = 'corona', n=15), na.value = 'grey90') +
  scale_color_manual(values = BuenColors::jdb_palette(name = 'corona', n=15), na.value = 'grey90') +
  theme_classic() +
...
# only consider expressed variants
results.df.filtered = results.df.filtered %>% filter(!is.na(gene))
...
ggplot(df, aes(x=n)) + geom_histogram(binwidth = 1, fill='lightblue') +
...
ggplot(df, aes(x=variants)) + geom_histogram(binwidth = 1, fill='lightblue') +
...
max(df$variants)
# 1.9% high variance
length(which(df$variants > 40)) / nrow(df)
# 0.0003% no variant 
