...
for (downsample in c(1,5,10,50,100,500,1000)) {
  ...
  write.table(file1.downsample, file = paste0('./data/artifical_mixing/barcodes_scRNAseq/CLL_relapse1_1_barcodes.',downsample), col.names = F, row.names = F, quote = F)
  write.table(file2.downsample, file = paste0('./data/artifical_mixing/barcodes_scRNAseq/CLL_relapse3_1_barcodes.',downsample), col.names = F, row.names = F, quote = F)
}