...
for (exp in c(1,5,10,50,100,500,1000)) {
  ...
  boo = data.table::fread(paste0('./data/artifical_mixing/vireo_no_reference/mixing_', exp, '/donor_ids.tsv'))
  boo$spikein = exp
  df = rbind(df, boo)
}
...
ggsave('./figures/artificial_mixing/20230328_vireo_no_reference_doublet_rate.svg', width = 1.5, height = 1.5)
...
ggplot(df %>% group_by(spikein) %>% 
  ...
  theme(legend.position = 'none',
        axis.text = element_text('Arial', size=10, color='black'),
        axis.title = element_text('Arial', size=10, color='black'))
ggsave('./figures/artificial_mixing/20230328_vireo_no_reference_unassignment_rate.svg', width = 1.5, height = 1.5)
...
ggplot(df %>% group_by(spikein) %>% 
  ...
  theme(legend.position = 'none',
        axis.text = element_text('Arial', size=10, color='black'),
        axis.title = element_text('Arial', size=10, color='black'))
ggsave('./figures/artificial_mixing/20230328_vireo_no_reference_cluster_ratio.svg', width = 1.5, height = 1.5)
...
ggplot() + 
  ...
  theme(legend.position = 'none',
        axis.text = element_text('Arial', size=10, color='black'),
        axis.title = element_text('Arial', size=10, color='black'))
ggsave('./figures/artificial_mixing/20230328_vireo_no_reference_cluster_distribution.svg', width = 1.5, height = 1.5)
