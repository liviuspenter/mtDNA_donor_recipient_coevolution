# process mgatk output and perform variant calling
...
samples = c('MART1_1','MART1_2','MART1_3')
atac.paths = c('./data/mixing/MART1/MART1_1/',
               './data/mixing/MART1/MART1_2/',
               './data/mixing/MART1/MART1_3/')
mgatk.paths = c('./data/mixing/MART1/MART1_1.mgatk/',
                './data/mixing/MART1/MART1_2.mgatk/',
                './data/mixing/MART1/MART1_3.mgatk/')
...
for (i in seq(1,length(samples))) {
  ...
  ggsave(plot = , p,filename = paste0('./figure_mixing/MART1/figures/qc/20211030_',samples[i],'_qc_plot1.png'), 
         width = 4, height = 4, dpi = 350)
  
  crc <- subset(crc, mtDNA_depth >= min_coverage)
  
  # save Seurat object and barcodes
  saveRDS(object = crc, file = paste0('./data/mixing/MART1/objects/20211030_',samples[i],'_seuratobject.rds'))
  write.table(data.frame(colnames(crc)), 
              file = paste0('./data/mixing/MART1/barcodes/',samples[i],'_barcodes.csv'), 
...
for (i in seq(1,length(samples))) {
  ...
  barcodes = read.table(file = paste0('./data/mixing/MART1/barcodes/',samples[i],'_barcodes.csv'))
...
ggsave(filename = paste0('./figure_mixing/MART1/figures/qc/20211030_MART1_strand_concordance_vmr.png'), plot = p, 
       width = 4, height = 4)

# Establish a filtered data frame of variants based on this processing
high.conf <- subset(combind.mutations, subset = n_cells_conf_detected >= 20 & strand_correlation >= 0.65 & vmr > 0.01)
write.table(high.conf, file = './data/mixing/MART1/mtDNA/20211030_MART1_high_confidence_variants.csv', sep = '\t', row.names = F, quote = F)

# calculate frequencies for all cells
combined.mutation.frequencies = data.frame(mutation = as.character())
for (i in seq(1,length(samples))) {
  crc = readRDS(file = paste0('./data/mixing/MART1/objects/20211030_',samples[i],'_seuratobject.rds'))
...
saveRDS(combined.mutation.frequencies, file = './data/mixing/MART1/mtDNA/20211030_MART1_combined_mutation_frequencies.rds')
