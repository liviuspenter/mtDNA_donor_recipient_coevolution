...
ha = columnAnnotation(cluster = so.12$manual.cluster[rownames(boo)],
                      col = list('cluster' = c('CLL' = 'orange', 'CD4 T cell' = 'lightblue', 'CD8 T cell' = 'darkblue', 'Myeloid' = 'darkgreen',
                                               'Cluster 6' = 'black')))
...
df = data.frame(variants.CLL1 = rowMeans(vafs.1[,variants.CLL1]),
...
# recipient variants
variants.CLL1.recipient = c('KCNA6:chr12:4920442:C/T', 'SH3RF1:chr4:170043260:C/T', 'ADGRL3:chr4:62862035:T/G', 'WNK1:chr12:966369:G/T',
...
df = data.frame(variants.CLL1.donor = rowMeans(vafs.1[cells.CLL1,variants.CLL1.donor]),
                variants.CLL1.recipient = rowMeans(vafs.1[cells.CLL1,variants.CLL1.recipient]))

ggplot(df, aes(x=variants.CLL1.donor, y=variants.CLL1.recipient)) + geom_point() +
