...
ha <- HeatmapAnnotation(
  celltype = variant.df[rownames(vafs), "manual.cluster"], col = list(celltype = cluster.colors),
  border = T, simple_anno_size = unit(5, "pt"), annotation_name_gp = gpar(fontsize = 8), gp = gpar(fontsize = 8),
  annotation_legend_param =
  )
Heatmap(t(vafs[, unique(c(rev(RHP.mutations), "11736T>C"))]),
  top_annotation = ha, raster_quality = 10, use_raster = T, border = T,
...
p <- DimPlot(so.13, group.by = "annotation", cols = c(
