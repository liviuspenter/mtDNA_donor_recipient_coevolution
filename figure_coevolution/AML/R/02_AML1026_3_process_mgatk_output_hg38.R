...
AML1026.3.mito <- ReadMGATK("./data/coevolution/1026_3.mgatk/")
AML1026.3.so <- CreateAssayObject(counts = AML1026.3.mito$counts)
AML1026.3.mgatk <- CreateSeuratObject(counts = AML1026.3.so, project = "AML1026.3.mgatk", assay = "mito")
rm(AML1026.3.so)
AML1026.3.mgatk <- AddMetaData(AML1026.3.mgatk, metadata = AML1026.3.mito$depth, col.name = "mtDNA_depth")

variable.sites <- IdentifyVariants(AML1026.3.mgatk, assay = "mito", refallele = AML1026.3.mito$refallele)

VariantPlot(variants = variable.sites)
high.conf <- variable.sites[which(variable.sites$n_cells_conf_detected > 5), ]

AML1026.3.mgatk <- AlleleFreq(object = AML1026.3.mgatk, variants = high.conf$variant, assay = "mito")
AML1026.3.vaf <- as.data.frame(t(as.data.frame(GetAssayData(AML1026.3.mgatk[["alleles"]]))))
write.csv(AML1026.3.vaf, file = "./data/coevolution/vafs/20220211_AML1026.3_mtDNA_vafs.csv")
write.table(variable.sites, file = "./data/coevolution/vafs/20220211_AML1026_3_variable_sites.csv")
...
p <- ggplot(variable.sites, aes(x = as.numeric(position), y = as.numeric(mean_coverage))) +
  geom_col() +
