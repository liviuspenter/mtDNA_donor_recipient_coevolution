...
variants.donor.recipient = c('NF1:chr17:29559932:C/A', 'TP53:chr17:7579801:G/C', 'SF3B1:chr2:198267770:G/GAA',
...
VARIANT_CUTOFF = 10
variant.df = data.frame(variants.1 = rowMeans(vafs[,variants.1]),
...
p = ggplot(variant.df, aes(x=variants.1, y=variants.2, color=annotation)) + geom_point(size=0.5) +
...
Heatmap(t(vafs[c(variant.df$barcode[which(variant.df$annotation == 'variant1')], 
                 variant.df$barcode[which(variant.df$annotation == 'variant2')]),
               c(variants.1, variants.2)]), top_annotation = ha, raster_quality = 10, use_raster = T, border = T,
...
for (celltype in unique(variant.df$manual.cluster)) {
  for (sample in unique(variant.df$sample)) {
    ...
    statistics.df = rbind(statistics.df, data.frame(celltype = celltype, sample = sample, variant1 = variant1, variant2 = variant2, 
