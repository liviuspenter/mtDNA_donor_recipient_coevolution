# process mgatk output and perform variant calling on mtDNA mutatios for samples for patient IST1  
# samples: IST1_1, IST1_2, IST2_1, IST2_2
...
samples = c('IST1_1','IST1_2', 'IST2_1', 'IST2_2')
atac.paths = c('./data/IST/IST1_1/',
               './data/IST/IST1_2/',
               './data/IST/IST2_1/',
               './data/IST/IST2_2/')
mgatk.paths = c('./data/IST/IST1_1.mgatk/',
                './data/IST/IST1_2.mgatk/',
                './data/IST/IST2_1.mgatk/',
                './data/IST/IST2_2.mgatk/')
...
for (i in seq(1,length(samples))) {
  ...
  ggsave(plot = , p,filename = paste0('./figure_IST/figures/qc/20220110_',samples[i],'_qc_plot1.png'), 
         width = 4, height = 4, dpi = 350)
  
  crc <- subset(crc, mtDNA_depth >= min_coverage)
  
  # save Seurat object and barcodes
  saveRDS(object = crc, file = paste0('./data/IST/objects/20220110_',samples[i],'_seuratobject.rds'))
  write.table(data.frame(colnames(crc)), 
              file = paste0('./data/IST/barcodes/',samples[i],'_barcodes.csv'), 
...
for (i in seq(1,length(samples))) {
  ...
  barcodes = read.table(file = paste0('./data/IST/barcodes/',samples[i],'_barcodes.csv'))
...
ggsave(filename = paste0('./figure_IST/figures/qc/20220110_IST1_2_strand_concordance_vmr.png'), plot = p, 
       width = 4, height = 4)

# Establish a filtered data frame of variants based on this processing
high.conf <- subset(combind.mutations, subset = n_cells_conf_detected >= 5 & strand_correlation >= 0.65 & vmr > 0.005)
write.table(high.conf, file = './data/IST/mtDNA/20220110_IST1_2_high_confidence_variants.csv', sep = '\t', row.names = F, quote = F)

# calculate frequencies for all cells
combined.mutation.frequencies = data.frame(mutation = as.character())
for (i in seq(1,length(samples))) {
  crc = readRDS(file = paste0('./data/IST/objects/20220110_',samples[i],'_seuratobject.rds'))
...
saveRDS(combined.mutation.frequencies, file = './data/IST/mtDNA/20220110_IST1_2_combined_mutation_frequencies.rds')