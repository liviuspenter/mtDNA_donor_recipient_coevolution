# comparison of donor- and recipient derived erythroid cells in bone marrow of patient IST1
...
### comparison of erythroid donor and recipient cells 
# protein
# GeneScore
# TF 
# mtDNA
...
donor.cells = c(df$bc[which(df$individual == 'donor' & df$manual.cluster == 'Erythroid' & df$sample == 'IST2_1')],
                df$bc[which(df$individual == 'donor' & df$manual.cluster == 'Erythroid' & df$sample == 'IST2_2')])
donor.cells.1 = df$bc[which(df$individual == 'donor' & df$manual.cluster == 'Erythroid' & df$sample == 'IST2_1')]
donor.cells.2 = df$bc[which(df$individual == 'donor' & df$manual.cluster == 'Erythroid' & df$sample == 'IST2_2')]

recipient.cells = df$bc[which(df$individual == 'recipient' & df$manual.cluster == 'Erythroid')]
...
ha = columnAnnotation(individual = c(rep('recipient', length(recipient.cells)),
                                     rep('donor1', length(donor.cells.1)),
                                     rep('donor2', length(donor.cells.2))),
                      col = list('individual' = c('donor1' = 'orange', 'donor2' = 'orange','recipient' = 'purple')),
...
TSB.heatmap = Heatmap(protein.mat[c('CD117','CD38','CD33','CD14','CD11c', 'CD16'), 
                     ...
                     column_split = factor(c(rep('recipient',length(recipient.cells)), 
                                             rep('donor1', length(donor.cells.1)),
                                             rep('donor2', length(donor.cells.2))),
                                           levels = c('recipient', 'donor1', 'donor2')), top_annotation = ha, 
...
diff.GS = rownames(volcano.data.GS)[which(abs(volcano.data.GS$log2fc) > 1 & -log10(volcano.data.GS$p.adj) > 10)]
...
ggsave('./figure_IST/figures/IST1/plots/20230417_IST1_erythroid_donor_recipient_GS.svg', width = 2, height = 2)
...
GS.heatmap = Heatmap(GS_matrix[c(diff.GS), c(recipient.cells, donor.cells)], 
                     ...
                     column_split = factor(c(rep('recipient',length(recipient.cells)), 
                                             rep('donor1', length(donor.cells.1)),
                                             rep('donor2', length(donor.cells.2))),
                                           levels = c('recipient', 'donor1', 'donor2')), 
                     col=col_fun, border=T, row_names_side = 'left', row_names_gp = gpar(fontsize=8), 
                     use_raster = T, raster_quality = 10)

IST.asap.mito$boo = 'none'
IST.asap.mito$boo[which(IST.asap.mito$cellNames %in% recipient.cells)] = 'recipient'
IST.asap.mito$boo[which(IST.asap.mito$cellNames %in% donor.cells.1)] = 'donor1'
IST.asap.mito$boo[which(IST.asap.mito$cellNames %in% donor.cells.2)] = 'donor2'
p=plotBrowserTrack(IST.asap.mito, groupBy = 'boo', useGroups = c('recipient', 'donor1', 'donor2'),
                   pal = c('donor1' = 'orange', 'donor2' = 'orange','recipient' = 'purple'),
                   downstream = 350000, geneSymbol = 'CD36')

svglite::svglite('./figure_IST/figures/IST1/browser_plots/20230425_IST1_erythroid_CD36.svg', width = 3, height = 2)
grid.draw(p$CD36)
dev.off()
# MotifMatrix
...
ggplot(volcano.data.TF, aes(x=log2fc, y=-log10(p.adj))) + 
  ...
  theme(axis.title = element_text('Arial', size=8, color='black'),
        axis.text = element_text('Arial', size=8, color='black'))
ggsave('./figure_IST/figures/IST1/plots/20230417_IST1_erythroid_donor_recipient_TF.svg', width = 2, height = 2)
...
svglite::svglite('./figure_IST/figures/IST1/heatmaps/20230417_IST1_erythroid_donor_recipient_heatmap.svg', width = 5, height = 3)
