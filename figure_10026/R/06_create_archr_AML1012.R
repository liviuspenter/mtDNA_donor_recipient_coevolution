# create ArchR object for AML1012 with sufficient mtDNA coverage
...
AML.input.files = createArrowFiles(inputFiles = c('./data/10026/AML1012_12/fragments.tsv.gz',
                                                  './data/10026/AML1012_34/fragments.tsv.gz'),
                                   sampleNames = c('AML1012_12','AML1012_34'),
...
AML.1012 = ArchRProject(ArrowFiles = AML.input.files, outputDirectory = './data/10026/AML.1012', copyArrows = F)

# filter doublets
AML.1012 = filterDoublets(AML.1012)

# basic clustering to identify major cell types
AML.1012 = addIterativeLSI(ArchRProj = AML.1012, useMatrix = 'TileMatrix', name = 'IterativeLSI', 
...
combined.mutation.frequencies = readRDS('./data/10026/mtDNA/20210429_AML1012_combined_mutation_frequencies.rds')
AML.1012.mito = subsetArchRProject(ArchRProj = AML.1012, 
                                     cells = AML.1012$cellNames[which(AML.1012$cellNames %in% colnames(combined.mutation.frequencies))],
                                     outputDirectory = './data/10026/AML.1012.mito/', threads = 12)
...
ADT.data = t(GetAssayData(TSB.so)[,gsub(AML.1012.mito$cellNames, pattern = '-1', replacement = '')])
for (protein in colnames(ADT.data)) {
  AML.1012.mito = addCellColData(AML.1012.mito, data = ADT.data[,protein], cells = rownames(AML.1012.mito), name = protein)
}
...
plotPDF(plotList = p, name = "ASAP.seq", ArchRProj = AML.1012.mito, addDOC = FALSE, width = 5, height = 5)

# add information on samples
cells.by.sample = read.table('./data/10026/20210611_cells_by_sample.csv')
AML.1012.mito = addCellColData(AML.1012.mito, 
                               data = cells.by.sample$sample[which(cells.by.sample$barcode %in% AML.1012.mito$cellNames)], 
                               cells = cells.by.sample$barcode[which(cells.by.sample$barcode %in% AML.1012.mito$cellNames)],
                               name = 'Sample.hash')
