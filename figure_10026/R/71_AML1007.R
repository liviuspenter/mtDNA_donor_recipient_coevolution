...
AML.input.files <- createArrowFiles(
  inputFiles = c(
    "./data/10026/Pool148_1/fragments.tsv.gz",
...
AML.1007 <- ArchRProject(ArrowFiles = AML.input.files, outputDirectory = "./data/10026/AML.1007", copyArrows = F)

# filter doublets
AML.1007 <- filterDoublets(AML.1007)

# basic clustering to identify CLL cells
AML.1007 <- addIterativeLSI(
  ArchRProj = AML.1007, useMatrix = "TileMatrix", name = "IterativeLSI",
...
combined.mutation.frequencies <- readRDS("./data/10026/mtDNA/20240516_AML1007_combined_mutation_frequencies.rds")
AML.1007.mito <- subsetArchRProject(
  ArchRProj = AML.1007,
  cells = AML.1007$cellNames[which(AML.1007$cellNames %in% colnames(combined.mutation.frequencies))],
  outputDirectory = "./data/10026/AML.1007.mito/", threads = 12
)
