...
chimerism.df = read.csv2('./data/mtDNA/20200302_AML1010_donor_recipient.csv', row.names = 1)
...
ggsave('./figures/heatmaps/20220304_AML1010_mtDNA_clusters.svg', width = 3, height = 5)
ggsave('./figures/heatmaps/20220304_AML1010_mtDNA_clusters.png', width = 3, height = 2.5, dpi=600)
...
for (s in c('AML1010_1', 'AML1010_2', 'AML1010_3', 'AML1010_4', 'AML1010_5')) {
  ...
  ggsave(paste0('./figures/plots/AML1010/20230331_mito_cluster_dynamics_', s, '.svg'), width=1.3, height = 2, plot = p)
}

for (s in c('AML1010_1', 'AML1010_2', 'AML1010_3', 'AML1010_4', 'AML1010_5')) {
  p=ggplot(df[which(df$sample == s),], aes(x=UMAP1, y=UMAP2, color=mito.cluster)) + geom_point(size=0.5) + 
    scale_color_manual(values = AML.1010.cluster.colors) + 
    theme_classic() +
    NoAxes() + NoLegend()
  ggsave(paste0('./figures/umaps/AML1010/20230331_',s,'_UMAP_mitoclusters.png'), width = 3, height = 3, dpi = 600, plot = p)
}
...
ggplot(boo, aes(x=label, y=blasts)) + geom_line(group=1) + 
  ...
  theme(legend.position = 'none',
        ...
        axis.title.x = element_blank(),
        axis.text.x = element_text(angle=90, hjust=1, vjust=0.5))
ggsave('./figures/plots/AML1010/20230331_clinical_course.svg', width = 1.3, height = 1.5)
...
ggplot(boo[which(boo$Var2 != 'none'),], 
  ...
  theme(legend.position = 'none', 
        ...
        axis.text = element_text('Arial', size=10, color='black'))
ggsave('./figures/plots/20220303_AML1010_mtDNA_clusters.svg', width = 1.3, height = 2)

DimPlot(mtDNA.so) +
  scale_color_manual(values = AML.1010.cluster.colors) + 
  NoAxes() + NoLegend()
ggsave('./figures/umaps/20220304_AML1010_mtDNA_clusters.png', width = 3, height = 3, dpi = 600)
...
svglite::svglite('./figures/heatmaps/20220303_AML1010_mtDNA_clusters.svg', width = 3, height = 2)
...
saveRDS(mtDNA.so, file = './data/objects/20200303_AML1010_mtDNA.so.rds')
