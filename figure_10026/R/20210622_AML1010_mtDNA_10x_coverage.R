...
samples = c('AML1010_1','AML1010_23','AML1010_45')
atac.paths = c('/Users/shaka87/dfci/asap_seq/data/AML1010_1/',
               '/Users/shaka87/dfci/asap_seq/data/AML1010_23/',
               '/Users/shaka87/dfci/asap_seq/data/AML1010_45/')
mgatk.paths = c('/Users/shaka87/dfci/asap_seq/data/AML1010_1.mgatk/',
                '/Users/shaka87/dfci/asap_seq/data/AML1010_23.mgatk/',
                '/Users/shaka87/dfci/asap_seq/data/AML1010_45.mgatk/')
...
for (i in seq(1,length(samples))) {
  ...
  ggsave(plot = , p,filename = paste0('./figures/qc/20210622_',samples[i],'_qc_plot1.png'), 
         width = 4, height = 4, dpi = 350)
  
  crc <- subset(crc, mtDNA_depth >= min_coverage)
  
  # save Seurat object and barcodes
  saveRDS(object = crc, file = paste0('./data/objects/20210622_',samples[i],'_seuratobject.rds'))
  write.table(data.frame(colnames(crc)), 
              file = paste0('./data/barcodes/',samples[i],'_barcodes.csv'), 
...
ggsave(filename = paste0('./figures/qc/20210622_AML1010_strand_concordance_vmr.png'), plot = p, 
       width = 4, height = 4)
...
write.table(high.conf, file = './data/mtDNA/20210622_AML1010_high_confidence_variants.csv', sep = '\t', row.names = F, quote = F)

# calculate frequencies for all cells
combined.mutation.frequencies = data.frame(mutation = as.character())
for (i in seq(1,length(samples))) {
  crc = readRDS(file = paste0('./data/objects/20210622_',samples[i],'_seuratobject.rds'))
...
saveRDS(combined.mutation.frequencies, file = './data/mtDNA/20210622_AML1010_combined_mutation_frequencies.rds')

# germline variants
high.conf.germline <- subset(combind.mutations, subset = n_cells_conf_detected >= 5 & strand_correlation >= 0.65 & vmr < 0.01)
write.table(high.conf.germline, file = './data/mtDNA/20210622_AML1010_high_confidence_variants.germline.csv', sep = '\t', row.names = F, quote = F)

# calculate germline frequencies for all cells
combined.mutation.frequencies = data.frame(mutation = as.character())
for (i in seq(1,length(samples))) {
  crc = readRDS(file = paste0('./data/objects/20210622_',samples[i],'_seuratobject.rds'))
  
  crc <- AlleleFreq(object = crc, variants = high.conf.germline$variant, assay = "mito")
  boo=as.data.frame(GetAssayData(crc, assay = 'alleles'))
  colnames(boo) = paste0(samples[i],'#',colnames(boo))
  boo$mutation = rownames(boo)
...
rownames(combined.mutation.frequencies) = combined.mutation.frequencies$mutation
combined.mutation.frequencies = combined.mutation.frequencies[,-1]
saveRDS(combined.mutation.frequencies, file = './data/mtDNA/20210622_AML1010_combined_mutation_frequencies.germline.rds')
