# discovery and tracking of AML subclones defined by mtDNA mutations in AML1026
...
chimerism.df = read.csv2('./data/10026/mtDNA/20200302_AML1026_donor_recipient.csv', row.names = 1)
...
ggsave('./figure_10026/figures/heatmaps/20220304_AML1026_mtDNA_clusters.svg', width = 3, height = 5)
ggsave('./figure_10026/figures/heatmaps/20220304_AML1026_mtDNA_clusters.png', width = 3, height = 2.5, dpi=600)
...
for (s in c('AML1026_1', 'AML1026_2', 'AML1026_3', 'AML1026_4')) {
  ...
  ggsave(paste0('./figure_10026/figures/plots/AML1026/20230331_mito_cluster_dynamics_', s, '.svg'), width=1.3, height = 2, plot = p)
}
...
for (s in c('AML1026_1', 'AML1026_2', 'AML1026_3', 'AML1026_4')) {
  p=ggplot(df[which(df$sample == s),], aes(x=UMAP1, y=UMAP2, color=mito.cluster)) + geom_point(size=0.5) + 
    ...
    theme_classic() +
    NoAxes() + NoLegend()
  ggsave(paste0('./figure_10026/figures/umaps/AML1026/20230331_',s,'_UMAP_mitoclusters.png'), width = 3, height = 3, dpi = 600, plot = p)
}
...
ggplot(boo, aes(x=label, y=blasts)) + geom_line(group=1) + 
  ...
  theme(legend.position = 'none',
        ...
        axis.title.x = element_blank(),
        axis.text.x = element_text(angle=90, hjust=1, vjust=0.5))
ggsave('./figure_10026/figures/plots/AML1026/20230331_clinical_course.svg', width = 1.3, height = 1.5)
...
ggplot(boo[which(boo$Var2 != 'none'),], 
  ...
  theme(legend.position = 'none', 
        ...
        axis.text = element_text('Arial', size=10, color='black'))
ggsave('./figure_10026/figures/plots/20220303_AML1026_mtDNA_clusters.svg', width = 1.2, height = 2)

DimPlot(mtDNA.so) +
  scale_color_manual(values = AML.1026.cluster.colors) +
  NoAxes() + NoLegend()
ggsave('./figure_10026/figures/umaps/20220304_AML1026_mtDNA_clusters.png', width = 3, height = 3, dpi = 600)

saveRDS(mtDNA.so, file = './data/objects/20220303_AML1026_mtDNA.so.rds')
