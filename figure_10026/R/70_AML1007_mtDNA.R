...
samples <- c("AML1007_1", "AML1007_5", "AML1007_6")
atac.paths <- c(
  "./data/10026/Pool148_1/",
  "./data/10026/Pool148_2/",
  "./data/10026/Pool148_12/"
)
mgatk.paths <- c(
  "./data/10026/Pool148_1.mgatk/final/",
  "./data/10026/Pool148_2.mgatk/final/",
  "./data/10026/Pool148_12.mgatk/final/"
)
...
for (i in seq(1, length(samples))) {
  ...
  crc_assay <- CreateChromatinAssay(
    counts = counts, sep = c(":", "-"), annotation = annotations, min.cells = 10, genome = "hg38",
  ...
  ggsave(
    plot = , p, filename = paste0("./figure_10026/figures/qc/20240516_", samples[i], "_qc_plot1.png"),
  ...
  saveRDS(object = crc, file = paste0("./data/10026/objects/20240516_", samples[i], "_seuratobject.rds"))
...
for (i in seq(1, length(samples))) {
  ...
  barcodes <- read.table(file = paste0("./data/10026/barcodes/", samples[i], "_barcodes.csv"))
...
ggsave(
  filename = paste0("./figure_10026/figures/qc/20240516_AML1007_strand_concordance_vmr.png"), plot = p,
  width = 4, height = 4
)
...
write.table(high.conf, file = "./data/10026/mtDNA/20240516_AML1007_high_confidence_variants.csv", sep = "\t", row.names = F, quote = F)

# calculate frequencies for all cells
combined.mutation.frequencies <- data.frame(mutation = as.character())
for (i in seq(1, length(samples))) {
  crc <- readRDS(file = paste0("./data/10026/objects/20240516_", samples[i], "_seuratobject.rds"))
  ...
  saveRDS(object = crc, file = paste0("./data/objects/20210429_", samples[i], "_seuratobject.rds"))
}
...
saveRDS(combined.mutation.frequencies, file = "./data/10026/mtDNA/20240516_AML1007_combined_mutation_frequencies.rds")
