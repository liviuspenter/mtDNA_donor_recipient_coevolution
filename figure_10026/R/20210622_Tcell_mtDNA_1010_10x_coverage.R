...
svglite::svglite('./figures/heatmaps/20210622_AML1010_mtDNA.svg', width = 4, height = 4.5)
...
ggsave('./figures/plots/20210622_chimerism_1010.svg', width = 1.5, height = 2.5)

write.table(chimerism.df, file = './data/mtDNA/20210712_Tcell_chimerism_1010.csv', sep = '\t', row.names = F)

# germline variants

combined.frequencies = readRDS(paste0('./data/mtDNA/20210622_',s,'_combined_mutation_frequencies.germline.rds'))
variants = read.table(paste0('./data/mtDNA/20210622_',s,'_high_confidence_variants.germline.csv'), header = T)

ComplexHeatmap::Heatmap(as.matrix(combined.frequencies[variants$variant,cells.ordered]), 
                        show_row_dend = F, show_column_dend = F, cluster_columns = T, cluster_rows = F,
                        show_column_names = F, show_row_names = T, show_heatmap_legend = F, 
                        row_title = '40 mtDNA mutations', row_title_gp = gpar(fontsize=10, color='black'),
                        column_title = paste0(ncol(combined.frequencies), ' T cells'), column_title_side = 'bottom', column_title_gp = gpar(fontsize=10, color='black'),
                        #top_annotation = ha.top, left_annotation = ha.left, 
                        col = col_fun, 
                        row_names_gp = gpar(fontsize = 8), use_raster = T)