...
for (p in unique(meta$Patient)) {
  files <- paste0("./data/CLL_RIC/", meta$Sample[which(meta$Patient == p)], ".mgatk")
  ...
  results.df$run <- stringr::str_split_fixed(results.df$sample, pattern = "/", n = 5)[, 5]
  results.df$run <- gsub(results.df$run, pattern = "\\.mgatk", replacement = "")
  results.df$patient <- stringr::str_split_fixed(results.df$run, pattern = "-", n = 3)[, 1]
  results.df$timepoint <- stringr::str_split_fixed(results.df$run, pattern = "-", n = 3)[, 3]

  variant.df.all <- data.frame()

  # make a list of variants
  variants <- unique(results.df$variant[which(results.df$heteroplasmy > 0.01 &
  ...
  colnames(mgatk.data.ATAC) <- gsub(pattern = "\\.bam", replacement = "", colnames(mgatk.data.ATAC))
  ...
  write.table(variant.df,
    file = paste0("./data/CLL_RIC/processed_together/", p, "_de_novo.csv"),
    sep = "\t", quote = F
  )
}
